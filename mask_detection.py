...
def main():
    ...
    while True:
        ...
        cv2.imshow("image processing project", image)
...
window = Tk()
window.title("Mask and Face detection")
window.geometry('600x400')
# window.configure(bg='#a1f0eb')

lbl = Label(window, text="Hello")
lbl.config(font=('Helvatical bold',20))

btn = Button(window, text="Run Program",bg="black", fg="white",command=main,padx=50)
 
#img = ImageTk.PhotoImage(Image.open("pp.png"))
imgLabel = Label(window,text="  ")
#imgLabel.image = img

#imgLabel.grid(column=3,row=1)
